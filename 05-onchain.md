...
最も長いblockchain上の少なくとも100の深さのブロックに含まれると、
...
node：
  ...
  またはHTLC outputを含むcommitment transactionのcommitment signatureを送った：（XXX: ？？？）
...
local nodeに資金が危機に瀕すると、remote nodeが一方的に閉じないようにblockchainを監視する必要がある。
...
誰もが無効なtransactions（不正な署名など）を生成する可能性があるので
...
node：
  - それのlocal commitment transaction発見時：
    ...
    （remote nodeのto_self_delayフィールドで指定された通りに）。
      ...
      （XXX: 前述のとおりcommitment transaction自体がspending transaction。別にほっておいてもよいがRationale参照）
...
node：
  ...
  その出力はirrevocably resolvedとみなされる。（XXX: 深度関係なくirrevocably resolvedにはならないだろう？？？）
  ...
  - commitment transaction HTLC outputがtimed outしていて、まだresolvedになっていない場合：
    ...
    - resolving transactionが合理的な深さに達すると：
      - 対応する着信HTLC（存在する場合）に失敗しなければならない。（XXX: 中継している反対側だろう）
...
nodeが支払いを抽出すると、HTLC-spending transaction自体の成り行きに関心はなくなる。
...
BOLT＃2に詳述されるように、
local nodeが、対応する着信HTLCをupdate_fail_htlcを使用して（おそらくpermanent_channel_failureという理由で）元に戻す前に
local HTLC-timeout transactionを使用して、HTLCをタイムアウトさせて（remote nodeがfulfillingして資金を請求するのを防ぐ）必要がある。
...
受信者は通常、完全にコミットするまでHTLCsを転送しないため（XXX: 別に転送してもいい違反ではないが？？？）、preimageを知らない。
従って、preimageを使用すると、この受信者が最終的なhop（XXX: final node）であることが明らかになってしまう。
...
（XXX: タイムアウトの前に転送して送金HTLCをコミットすればpreimageを受信して一連の送金を成功できるが、
タイムアウトさせて中断させてもいい、ということか？？？）
3. 受信者は、offered HTLCと引き換えに、送金HTLCをコミットしている。
この場合、受信側は、送金HTLCから受信したpreimageを使用する必要がある。
そうでなければ、それは入金を償還せずに出金を送ることによって資金を失うだろう。
(XXX: 送金HTLCはpreimageによって下流に渡させる。offered HTLCをタイムアウトさせると上流が受け取るので中継者は資金を失う)
...
local node：
  ...
  - そうでなければ：
    - remote nodeがHTLCをirrevocably committedしていない場合：（XXX: 上記1のケースか？）
      - それを使ってoutputをresolveにしてはいけない。（XXX: タイムアウトさせる？）  
  - 都合の良いアドレスを使ってHTLC-success transaction outputを解決すべきである。
  - HTLC-success transaction outputを使う前に、OP_CHECKSEQUENCEVERIFYの遅延が経過するまで待たなければならない。
...
（XXX: 複数ありうるのは古いものがまだrevokeされていないから。２つ以上もありうる？）
...
local nodeはremote nodeのper_commitment_pointを所有するため（XXX: option_data_loss_protectだとこれができるのか）、
従って、自己の資金をサルベージするために、そのtransactionのための、自身のremotepubkeyを導出できる。
（XXX: per_commitment_pointを貰わないとremotepubkeyもprivkeyも導出できない。remoteと言っているが自身のもの）
注：このシナリオでは、nodeはHTLCsをサルベージすることができない。（XXX: 署名を持っていないから？）
...
（XXX: commitment_signedを送っているが、revoke_and_ackを送っていなければ、
remote nodeが２つの有効なcommitment transactionを持つ可能性はあるが、
それがHTLCsが現れない原因の説明に記述されている理由がわからない）
...
local node：（XXX: 有効なcommitment transactionが複数ある場合を想定しなければならない）
  - commitment transaction HTLC outputがpayment preimageを使用して費やされた場合：
  ...
  - このcommitment transactionではoutputを持たないコミット済みのHTLCのためには：  
    ...
    - そうでなければ：（XXX: インデントが一つ上？）  
      - 有効なcommitment transaction（XXX: 複数ありうる）がHTLCに対応するoutputを含まない場合：
        - すぐに失敗しても良い。
...
（XXX: タイムアウトになる前に費やす。payment preimageで？）
...
（remote nodeがHTLC-success transactionを使用しないように（XXX: タイムアウト後に受信した））。
BOLT #2に詳述されるように。
...
nodeが失敗する前に（XXX: ？？？）、HTLCを含むcommitment transactionに切り替わらないか確認する必要がある。
着信HTLCがそれ自身のタイムアウトの前に失敗する要件は依然として上限として適用される。（XXX: ？？？）
...
commitment transactionを発見したら、
funding transaction outputをresolvedにする。
...
local node：
  - それがper_commitment_secretを公開された、commitment transactionをブロードキャストしてはならない。
  ...
  - local nodeのoffered HTLCsを次の3つの方法のいずれかでresolveしなければならない。
  ...
  - remote nodeのoffered HTLCsを次の2つの方法のいずれかでresolveしなければならない。
  ...
  - もしそれがまだ既知ではない場合、transaction input witnessからpayment preimageを抽出すべきである。
...
標準のサイズ制限を下回る。
（XXX: 共通の一つのtransactionで全部のoutputsを解決する。それが効率がいいから）
...
penalty transactionは、オプションとして、commitment transactionのto_remote outputを費やすことがある。
（例えば、feesの支払い総額を減らすために）
（XXX: このto_remoteは自分へのP2WPKH。これもこのtransactionに追加した方がいいということか。
独立したtransactionで回収するよりもfeesの節約になるということ？）
これを行うには、P2WPKH witnessと追加のtxinputを含める必要があり、その結果、108 + 164 = 272バイトが追加される。
...
最悪の場合のシナリオでは、ノードは着信HTLCのみを保持し、
HTLC-timeout transactionsは公開されないため、
nodeはcommitment transactionから消費する。
（XXX: received HTLCsのみと言ってくれた方がわかりやすい）
