...
ルーティングスキーマはSphinxの構文に基づいており、hopごとのペイロードで拡張されている。
（XXX: ？）
...
このエンドツーエンド認証を使用すると、
各hopはper_hopで指定された値でHTLCパラメータをクロスチェックし、
...
フィールドの説明：
   ...
   * outgoing_cltv_value：パケットを運ぶ送信HTLCが持たなければならないCLTV値。
     ...
     このフィールドを含めることによりhopは、origin nodeによって指定された情報と転送されるHTLCのパラメータの両方を認証することができ、
     origin nodeが現在のcltv_expiry_delta値を使用していることを保証することができます。
...
最後から2番目のノードによるprobing attacks（XXX: 探査攻撃）の可能性もなくなる。（XXX: なぜ？）
そのような攻撃は、そうでなければ、受信ピアが異なる量/期限を持つHTLCを再送することによって、
...
そのhopの送信者のephemeral keyとhopのnode ID keyとの間のElliptic-curve Diffie-Hellmanを用いて確立する。
...
onionを構築するために、送信者は、最初のhop ek_1のためのephemeral private keyをsessionkeyで初期化すると、
そこから対応するephemeral public key epk_1をsecp256k1 base pointを乗算して導出する。
ルートに沿ったk hopsのそれぞれについて、送信側は次に、次のhop ek_{k+1}のshared secret ss_kとephemeral keyを繰り返し計算する。
...
（XXX: 区切り）
 ...
 - 送信者は、
 curve pointを得るために、hopのpublic keyとephemeral private keyでECDHを実行する、
...
r個の32バイトephemeral public keys、
r個の32バイトshared secrets、
r個の32バイトblinding factors、
...
パケット生成は、versionバイト、最初のhopのephemeral pubkey、最初のhopのHMAC、
...
次の1300バイトは送信パケット用のhops_dataである。
...
パケットを受信すると、processing nodeは、経路情報およびhopごとのペイロードから、それに向けられた情報を抽出する。
...
origin node：
  ...
  - そうではなく、intermediate hopがエラーを返している：
    ...
    - そうでなければ：
      ...
      - そうでなければ：
        - erring nodeからの送信チャネルを考慮から削除すべきである。
        - PERMビットがセットされていない場合：
